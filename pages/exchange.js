...
const Exchange = () => {
  ...
  const purchase = async () => {
    try {
      ...
      alert("You have successfully purchased ATERETA tokens");
    } catch (error) {
      console.error(error);
  ...
  const sell = async () => {
    try {
      ...
      alert("You have successfully sold ATERETA tokens");
    } catch (error) {
      console.error(error);
      alert("Error purchasing tokens");
    }
  ...
  return (
    <div>
      <Head>
        <title>Exchange ATERETA tokens</title>
      </Head>
      <div className="exchange__container">
        <h1>BUY ATERETA Tokens</h1>
        <input
          type="number"
